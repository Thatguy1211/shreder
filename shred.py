        ...
        if folder == 'file':
            ...
            if shred_level in shred_levels:
                try:
                    ...
                    print(f"{color_start} Oooops! Looks like something went wrong, please make sure that what you were trying to corrupt was a real file, or that this program has ADMIN privileges")
    answers = ['y', 'yes']
    visualize = input(f"{color_start} Would you like to use the byte visualizer?\n")
    if visualize in answers:
        try:
            os.system("python ./byte_visualizer.py")
        except:
            print(f"{color_start} Something went wrong, you are going to need to open the byte visualizer manually, we're really sorry")
